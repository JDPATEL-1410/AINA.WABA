...
export const Inbox: React.FC = () => {
  ...
  useEffect(() => {
    // Config: Use VITE_API_URL if defined (Split Deployment), otherwise default logic
    const env = (import.meta as any)?.env;
    const socketUrl = env?.VITE_API_URL 
        ? env.VITE_API_URL 
        : (env?.PROD ? undefined : 'http://localhost:3000');
  ...
  const MessageBubble: React.FC<{ msg: Message }> = ({ msg }) => {
      ...
      return (
          <div className={`flex w-full mb-3 ${isMe ? 'justify-end' : 'justify-start'}`}>
              <div className={`relative max-w-[65%] px-3 py-1.5 rounded-lg shadow-sm text-sm group ${
